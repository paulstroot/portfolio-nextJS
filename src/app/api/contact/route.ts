...
export async function POST(req: Request) {
  try {
    ...
    const sanitizedTo = {
      ...
      name: body.to.name
        ? escapeHtml(String(body.to.name).trim().substring(0, 100))
        : undefined, // Limit name length and escape HTML
    };
    ...
    const sanitizedSubject = escapeHtml(
      String(body.subject).trim().substring(0, 255)
    );
