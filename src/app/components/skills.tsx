...
import { SkillCategoryItem, SkillCategoryQueryResult, SkillItem, SkillQueryResult } from "../types"
...
export default async function Skills() {
  ...
  const sortedCategories = categories.items.sort((a, b) => a.fields.order - b.fields.order)
  const allSkills = await getAllSkills()

  return (
    <section id="skills" className="skills bg-primary text-primary-contrast py-8 my-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2  gap-4">
          {sortedCategories.map((cat: SkillCategoryItem, i: number) => {
            const theseSkills = allSkills.items
              .filter((s) => {
                return s.fields?.category.find((c) => c.fields?.name == cat.fields.name)
              })
            ...
            return (
              ...
              >
                <h3 className="text-xl font-semibold mb-4">{escapeHtml(cat.fields.name)}</h3>
                        ...
                        <div className="flex flex-col items-center justify-center text-xs text-center">
                          {icon?.fields?.file?.url && (
                            <Image
                              ...
                              alt={escapeHtml(icon?.fields?.file?.title ?? name)}
                              width={32}
                              height={32}
