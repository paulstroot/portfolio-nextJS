...
export default function ContactForm() {
	...
	const validateField = (name: string, value: string) => {
		...
		switch (name) {
			...
			case "email": {
				...
				else if (!isValidEmail(value.trim())) current.email = "Enter a valid email address."
			...
			case "message": {
				...
				else if (value.trim().length < 5) current.message = "Message must be at least 5 characters."
	...
	if (success) {
		return (
			<div className="text-primary-contrast" role="alert" aria-live="polite" aria-atomic="true">
				<h3>Thank you</h3>
	...
	if (error) {
		return (
			<div className="text-primary-contrast" role="alert" aria-live="assertive" aria-atomic="true">
				<h3>Uh Oh!</h3>
				<p>There has been an error. Please try again.</p>
				<p className="text-error small">{escapeHtml(error).substring(0, 200)}</p>
			</div>
		)
	}
	return (
		...
		>
			<div className="grid md:grid-cols-2 md:gap-6">
				<div className="relative z-0 w-full mb-5 group">
					<input
						...
						className={`peer ${validationErrors.firstname ? "invalid" : "valid"}`}
						placeholder=" "
						required
					...
					{validationErrors.firstname && (
						<span id="firstname-error" className="error-message" role="alert" aria-live="assertive">
							{validationErrors.firstname}
						</span>
					)}
				...
				<div className="relative z-0 w-full mb-5 group">
					<input
						...
						className={`peer ${validationErrors.lastname ? "invalid" : "valid"}`}
						placeholder=" "
						required
					...
					{validationErrors.lastname && (
						<span id="lastname-error" className="error-message" role="alert" aria-live="assertive">
							{validationErrors.lastname}
						</span>
					)}
			...
			<div className="relative z-0 w-full mb-5 group">
				...
				{validationErrors.phone && (
					<span id="phone-error" className="error-message" role="alert" aria-live="assertive">
						{validationErrors.phone}
					</span>
				)}
			...
			<div className="relative z-0 w-full mb-5 group">
				...
				{validationErrors.message && (
					<span id="message-error" className="error-message" role="alert" aria-live="assertive">
						{validationErrors.message}
					</span>
				)}
